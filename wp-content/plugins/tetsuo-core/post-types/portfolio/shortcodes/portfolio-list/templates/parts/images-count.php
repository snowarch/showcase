...
if ( $item_type === 'gallery' ) {
	$gallery = tetsuo_core_get_portfolio_single_media();
	if ( $enable_count_images === 'yes' && is_array( $gallery ) && count( $gallery ) > 0 ) {
		...
		if ( ! empty( $featured ) ) {
			$counter ++;
			if ( ($item_style === 'standard-switch-images' || $item_style ==='gallery-switch-images') && ! empty( $switch_featured_image ) ) {
				$counter ++;
			}
		}
		...
		<div class="edgtf-gli-number-of-images-holder">
			<span><?php echo esc_attr( $counter ) . esc_html__( ' pics', 'tetsuo-core' ); ?></span>
		</div>
	<?php }
}