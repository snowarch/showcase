<?php
namespace TetsuoCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'tetsuo-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    tetsuo_edge_icon_collections()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    'heading'    => esc_html__( 'Size', 'tetsuo-core' ),
							    'value'      => array(
								    esc_html__( 'Tiny', 'tetsuo-core' )   => 'edgtf-icon-tiny',
								    esc_html__( 'Small', 'tetsuo-core' )  => 'edgtf-icon-small',
								    esc_html__( 'Medium', 'tetsuo-core' ) => 'edgtf-icon-medium',
								    esc_html__( 'Large', 'tetsuo-core' )  => 'edgtf-icon-large',
								    esc_html__( 'Huge', 'tetsuo-core' )   => 'edgtf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'tetsuo-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
							    ...
							    'value'      => array(
								    esc_html__( 'Normal', 'tetsuo-core' ) => 'edgtf-normal',
								    esc_html__( 'Circle', 'tetsuo-core' ) => 'edgtf-circle',
								    esc_html__( 'Square', 'tetsuo-core' ) => 'edgtf-square'
							    ),
							    'save_always' => true
						    ),
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'tetsuo-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'tetsuo-core' ),
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'tetsuo-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'icon_color',
							    'heading'    => esc_html__( 'Icon Color', 'tetsuo-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'border_color',
							    'heading'    => esc_html__( 'Border Color', 'tetsuo-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'border_width',
							    'heading'    => esc_html__( 'Border Width', 'tetsuo-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'tetsuo-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'hover_border_color',
							    'heading'    => esc_html__( 'Hover Border Color', 'tetsuo-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'hover_background_color',
							    'heading'    => esc_html__( 'Hover Background Color', 'tetsuo-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'        => 'textfield',
							    'param_name'  => 'margin',
							    'heading'     => esc_html__( 'Margin', 'tetsuo-core' ),
							    'description' => esc_html__( 'Insert margin in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'tetsuo-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'icon_animation',
							    'heading'    => esc_html__( 'Icon Animation', 'tetsuo-core' ),
							    'value'      => array_flip( tetsuo_edge_get_yes_no_select_array( false ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'icon_animation_delay',
							    'heading'    => esc_html__( 'Icon Animation Delay (ms)', 'tetsuo-core' ),
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'tetsuo-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'tetsuo-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$default_atts = array_merge( $default_atts, tetsuo_edge_icon_collections()->getShortcodeParams() );
		$params       = shortcode_atts( $default_atts, $atts );
		
		$iconPackName = tetsuo_edge_icon_collections()->getIconCollectionParamNameByKey( $params['icon_pack'] );
		...
		$html = tetsuo_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'edgtf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'edgtf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . tetsuo_edge_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'edgtf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . tetsuo_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . tetsuo_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . tetsuo_edge_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . tetsuo_edge_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'edgtf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . tetsuo_edge_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
