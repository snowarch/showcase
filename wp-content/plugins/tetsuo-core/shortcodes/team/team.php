...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = tetsuo_edge_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'tetsuo-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'tetsuo-core' ) . $x . esc_html__( ' Link', 'tetsuo-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => tetsuo_edge_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'tetsuo-core' ) . $x . esc_html__( ' Target', 'tetsuo-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'tetsuo-core' ) => '_self',
					esc_html__( 'New Window', 'tetsuo-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'tetsuo-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image', 'tetsuo-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'tetsuo-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
							array(
							...
							array(
								...
								'heading'    => esc_html__( 'Name', 'tetsuo-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'tetsuo-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'tetsuo-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
							...
							array(
								...
								'heading'    => esc_html__( 'Position Color', 'tetsuo-core' ),
								'dependency' => array( 'element' => 'team_position', 'not_empty' => true )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_social_icon_pack',
								'heading'     => esc_html__( 'Social Icon Pack', 'tetsuo-core' ),
								'value'       => array_merge( array( '' => '' ), tetsuo_edge_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( tetsuo_edge_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = tetsuo_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = tetsuo_edge_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = tetsuo_edge_execute_shortcode( 'edgtf_icon', $team_icon_params );
				}
			}
		}
