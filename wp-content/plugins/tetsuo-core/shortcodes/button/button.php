<?php
namespace TetsuoCore\CPT\Shortcodes\Button;
...
class Button implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Button', 'tetsuo-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'value'      => array(
									...
									esc_html__( 'Huge', 'tetsuo-core' )    => 'huge'
								),
						...
						tetsuo_edge_icon_collections()->getVCParamsArray( array(), '', true ),
						array(
							array(
								'type'       => 'colorpicker',
								'param_name' => 'color',
								'heading'    => esc_html__( 'Color', 'tetsuo-core' ),
								'group'      => esc_html__( 'Design Options', 'tetsuo-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'hover_color',
								'heading'    => esc_html__( 'Hover Color', 'tetsuo-core' ),
								'group'      => esc_html__( 'Design Options', 'tetsuo-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Font Size (px)', 'tetsuo-core' ),
								'group'      => esc_html__( 'Design Options', 'tetsuo-core' )
							),
							array(
								'type'        => 'dropdown',
							...
							array(
								'type'        => 'textfield',
								'param_name'  => 'padding',
								'heading'     => esc_html__( 'Button Padding', 'tetsuo-core' ),
								'description' => esc_html__( 'Insert padding in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'tetsuo-core' ),
								'dependency'  => array( 'element' => 'type', 'value' => array( 'solid', 'outline' ) ),
								'group'       => esc_html__( 'Design Options', 'tetsuo-core' )
							)
						)
	...
	public function render( $atts, $content = null ) {
		...
		return tetsuo_core_get_shortcode_module_template_part( 'templates/' . $params['html_type'], 'button', '', $params );
	}
	...
	private function getButtonStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . tetsuo_edge_filter_px( $params['font_size'] ) . 'px';
		}
