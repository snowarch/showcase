...
$row_content_width = $anchor = $content_text_aligment = $row_vertical_text = $row_vertical_text_output = $simple_background_color = $simple_background_image = $background_image_position = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = '';
...
if(!empty($row_vertical_text)) {

    $row_vertical_text_style = array();

    if(!empty($row_vertical_text_color)){
        $row_vertical_text_style[] = 'color:' . esc_html($row_vertical_text_color);
    }

    $css_classes[] = 'edgtf-row-has-vertical-text';

    $row_vertical_text_wrapper_classes = '';

    if (!empty($row_vertical_text_align)) {
        $row_vertical_text_wrapper_classes .= ' edgtf-row-vertical-text-align-'.$row_vertical_text_align;
    }

    $row_vertical_text_holder_style = array();

    if(!empty($row_vertical_text_holder_left_offset)){
        if($row_vertical_text_align == 'left') {
            $row_vertical_text_holder_style[] = 'left:' . esc_html($row_vertical_text_holder_left_offset);
        } else {
            $row_vertical_text_holder_style[] = 'left: calc(100% - ' . esc_html($row_vertical_text_holder_left_offset) . ')';
        }
    }

    $row_vertical_text_output = '<div class="edgtf-row-vertical-text-holder' . esc_attr($row_vertical_text_wrapper_classes) . '" ' . tetsuo_edge_get_inline_style($row_vertical_text_holder_style) . '>';
    $row_vertical_text_output .= '<div class="edgtf-row-vertical-text-holder-inner">';
    $row_vertical_text_output .= '<span class="edgtf-row-vertical-text" '. tetsuo_edge_get_inline_style($row_vertical_text_style) .'>';
    $row_vertical_text_output .= $row_vertical_text;
    $row_vertical_text_output .= '</span>';
    $row_vertical_text_output .= '</div>';
    $row_vertical_text_output .= '</div>';
}
...
if ( $row_content_width === 'grid' ) {
	$edgtf_row_wrapper_start .= '<div class="edgtf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . tetsuo_edge_get_inline_style( $edgtf_grid_row_styles ) . '><div class="edgtf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . tetsuo_edge_get_inline_style( $edgtf_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
if($row_vertical_text_output !== '') {
    $output .= $row_vertical_text_output;
}
$output .= '</div>';
$output .= $edgtf_row_wrapper_end;
$output .= $after_output;

echo tetsuo_edge_get_module_part( $output );
