...
class EdgeSkin extends TetsuoEdgeClassSkinAbstract {
    /**
     * Skin constructor. Hooks to tetsuo_edge_admin_scripts_init and tetsuo_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('tetsuo_edge_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('tetsuo_edge_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('tetsuo_edge_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('tetsuo_edge_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('tetsuo_edge_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('tetsuo_edge_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			tetsuo_edge_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            tetsuo_edge_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = tetsuo_edge_get_admin_tab();
        $active_page = $tetsuo_edge_global_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see TetsuoEdgeClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param TetsuoEdgeClassAdminPage $page current page to load
     * @see TetsuoEdgeClassSkinAbstract::loadTemplatePart()
     */
