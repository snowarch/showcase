<?php
namespace TetsuoCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'tetsuo' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'tetsuo' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'tetsuo' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'tetsuo' ),
							'value'       => array(
								esc_html__( 'Title', 'tetsuo' )          => 'title',
								esc_html__( 'Featured Image', 'tetsuo' ) => 'featured_image',
								esc_html__( 'Category', 'tetsuo' )       => 'category',
								esc_html__( 'Excerpt', 'tetsuo' )        => 'excerpt',
								esc_html__( 'Price', 'tetsuo' )          => 'price',
								esc_html__( 'Rating', 'tetsuo' )         => 'rating',
								esc_html__( 'Add To Cart', 'tetsuo' )    => 'add_to_cart',
								esc_html__( 'Tag', 'tetsuo' )            => 'tag',
								esc_html__( 'Author', 'tetsuo' )         => 'author',
								esc_html__( 'Date', 'tetsuo' )           => 'date',
							),
							'admin_label' => true
						),
						array(
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'tetsuo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'tetsuo' ),
							'value'      => array(
								...
								esc_html__( 'Shop Single', 'tetsuo' )    => 'woocommerce_single',
								esc_html__( 'Shop Thumbnail', 'tetsuo' ) => 'woocommerce_thumbnail'
							),
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'tetsuo' ),
							'value'       => array_flip( tetsuo_edge_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'tetsuo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'tetsuo' ),
							'value'      => array(
								esc_html__( 'Default', 'tetsuo' ) => '',
								esc_html__( 'White', 'tetsuo' )   => 'white',
								esc_html__( 'Dark', 'tetsuo' )    => 'dark'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'tetsuo' ),
							'value'      => array(
								esc_html__( 'Default', 'tetsuo' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'tetsuo' ) ),
							'group'      => esc_html__( 'Product Info Style', 'tetsuo' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="edgtf-pi-title entry-title" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="edgtf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="edgtf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="edgtf-pi-excerpt"><p itemprop="description" class="edgtf-pi-excerpt-item" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="edgtf-pi-price" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="edgtf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "tetsuo" ), $average ) . '" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'tetsuo_edge_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="edgtf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="edgtf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="edgtf-pi-author" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="edgtf-pi-date" ' . tetsuo_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'tetsuo' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'tetsuo' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'tetsuo' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'tetsuo' ) . ': ' . $product_id;
