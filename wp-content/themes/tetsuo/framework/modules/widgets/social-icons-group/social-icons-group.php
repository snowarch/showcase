...
class TetsuoEdgeClassClassIconsGroupWidget extends TetsuoEdgeClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_social_icons_group_widget',
			esc_html__( 'Tetsuo Social Icons Group Widget', 'tetsuo' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'tetsuo' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			tetsuo_edge_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'tetsuo' ),
					'options' => array(
						''             => esc_html__( 'Default', 'tetsuo' ),
						'square-icons' => esc_html__( 'Square', 'tetsuo' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'tetsuo' ),
					'description' => esc_html__( 'This applies to the square layout', 'tetsuo' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'tetsuo' ),
						'light-skin' => esc_html__( 'Light Skin', 'tetsuo' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'tetsuo' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'tetsuo' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="edgtf-social-icon-widget-holder edgtf-icon-has-hover" <?php echo tetsuo_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php tetsuo_edge_inline_style( $icon_styles ) ?>
