...
class HeaderCentered extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->logoAreaHeight     = tetsuo_edge_set_default_logo_height_for_header_types();
			$this->menuAreaHeight     = tetsuo_edge_set_default_menu_height_for_header_types();
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['logo_area_in_grid'] = tetsuo_edge_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' ? true : false;
		$parameters['menu_area_in_grid'] = tetsuo_edge_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' ? true : false;
		
		$parameters = apply_filters( 'tetsuo_edge_filter_header_centered_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$logo_background_color             = tetsuo_edge_get_meta_field_intersect( 'logo_area_background_color', $id );
		$logo_background_transparency      = tetsuo_edge_get_meta_field_intersect( 'logo_area_background_transparency', $id );
		$logo_grid_background_color        = tetsuo_edge_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = tetsuo_edge_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		$menu_background_color             = tetsuo_edge_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = tetsuo_edge_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		$menu_grid_background_color        = tetsuo_edge_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = tetsuo_edge_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( ( $sliderExists && tetsuo_edge_is_top_bar_enabled() )
				     || tetsuo_edge_is_top_bar_enabled() && tetsuo_edge_is_top_bar_transparent()
				) {
					$transparencyHeight += tetsuo_edge_get_top_bar_height();
				}
			}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$logo_background_color             = tetsuo_edge_options()->getOptionValue( 'logo_area_background_color' );
		$logo_background_transparency      = tetsuo_edge_options()->getOptionValue( 'logo_area_background_transparency' );
		$logo_grid_background_color        = tetsuo_edge_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = tetsuo_edge_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		$menu_background_color             = tetsuo_edge_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = tetsuo_edge_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = tetsuo_edge_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = tetsuo_edge_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( tetsuo_edge_is_top_bar_enabled() && tetsuo_edge_is_top_bar_completely_transparent() ) {
					$transparencyHeight += tetsuo_edge_get_top_bar_height();
				}
			}
	...
	public function calculateHeaderHeight() {
		...
		if ( tetsuo_edge_is_top_bar_enabled() ) {
			$headerHeight += tetsuo_edge_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = tetsuo_edge_get_meta_field_intersect( 'header_behaviour' );
		if ( ! in_array( $header_behavior, array( 'sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up' ) ) ) {
			$perPageVars['edgtfHeaderTransparencyHeight'] = $this->headerHeight - ( tetsuo_edge_get_top_bar_height() + $this->heightOfCompleteTransparency );
		} else {
