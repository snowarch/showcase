...
/*
   Class: TetsuoEdgeClassMultipleImages
   A class that initializes Edge Multiple Images
*/

class TetsuoEdgeClassMultipleImages implements iTetsuoEdgeInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$tetsuo_edge_global_Framework->edgtMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="edgtf-gallery-uploader">
								<a class="edgtf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'tetsuo' ); ?></a>
								<a class="edgtf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'tetsuo' ); ?></a>
							</div>
...
class TetsuoEdgeClassTwitterFramework implements iTetsuoEdgeInterfaceRender {
	public function render( $factory ) {
		$twitterApi = TetsuoTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'tetsuo' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'tetsuo' ) : esc_html__( 'Connect with Twitter', 'tetsuo' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="edgtf-page-form-section" id="edgtf_enable_social_share">
			<div class="edgtf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'tetsuo' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'tetsuo' ); ?></p>
			</div>
			<div class="edgtf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class TetsuoEdgeClassInstagramFramework implements iTetsuoEdgeInterfaceRender {
    public function render( $factory ) {
        ...
        if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
            ...
            $message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'tetsuo' );
        }
        ...
        if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
            ...
            $message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'tetsuo' );
        }
        ...
        if ( ! empty( $_GET['disconnect'] ) ) {
            ...
            $message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'tetsuo' );
        ...
        <div class="edgtf-page-form-section" id="edgtf_enable_social_share">
            <div class="edgtf-field-desc">
                <h4><?php esc_html_e( 'Connect with Instagram', 'tetsuo' ); ?></h4>
                <p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'tetsuo' ); ?></p>
            </div>
            <div class="edgtf-section-content">
                <div class="container-fluid">
                    <?php
                    ...
                    if ( $instagram_user_id ) { ?>
                        <div class="row">
                            <div class="col-lg-12">
                                <p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'tetsuo' );
                    ...
                    <div class="row">
                        <?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'tetsuo' ); ?></a>
                            </div>
                        <?php } else if ( empty( $connection_type ) ) { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'tetsuo' ); ?></a>
                            </div>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'tetsuo' ); ?></a>
                            </div>
                        <?php } else { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'tetsuo' ) ?></a>
                            </div>
                        <?php } ?>
                    </div>
                </div>
...
class TetsuoEdgeClassRepeater implements iTetsuoEdgeInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="edgtf-repeater-table-heading">
					<div class="edgtf-repeater-fields-holder">
						<div class="edgtf-repeater-table-cell edgtf-repeater-sort"><?php esc_html_e( 'Order', 'tetsuo' ) ?></div>
						...
						<div class="edgtf-repeater-table-cell edgtf-repeater-remove"><?php esc_html_e( 'Remove', 'tetsuo' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="edgtf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="edgtf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = tetsuo_edge_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class TetsuoEdgeClassFieldAddress extends TetsuoEdgeClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Tetsuo Options', 'tetsuo' ); ?></p>
							<?php } ?>
...
class TetsuoEdgeClassFieldIcon extends TetsuoEdgeClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = tetsuo_edge_icon_collections()->getIconCollectionsEmpty();
