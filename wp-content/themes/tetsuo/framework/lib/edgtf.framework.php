...
/*
   Class: TetsuoEdgeClassFramework
...
class TetsuoEdgeClassFramework {
	...
	private function __construct() {
		$this->edgtOptions         = TetsuoEdgeClassOptions::get_instance();
		$this->edgtMetaBoxes       = TetsuoEdgeClassMetaBoxes::get_instance();
		$this->edgtTaxonomyOptions = TetsuoEdgeClassTaxonomyOptions::get_instance();
		$this->edgtUserOptions     = TetsuoEdgeClassUserOptions::get_instance();
		$this->edgtDashboardOptions = TetsuoEdgeClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( TetsuoEdgeClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class TetsuoEdgeClassSkinManager
 *
...
class TetsuoEdgeClassSkinManager {
	...
	/**
	 * @see TetsuoEdgeClassSkinManager::setSkin()
	 */
...
/**
 * Class TetsuoEdgeClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class TetsuoEdgeClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( tetsuo_edge_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.edgtSCIcon = '<?php echo tetsuo_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.edgtSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'tetsuo' ) ?>';
		</script>
	<?php }
...
class TetsuoEdgeClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $tetsuo_edge_global_options ) ) {
			return $tetsuo_edge_global_options[ $key ];
...
/*
   Class: TetsuoEdgeClassAdminPage
   A class that initializes Edge Admin Page
*/
class TetsuoEdgeClassAdminPage implements iTetsuoEdgeInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new TetsuoEdgeClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iTetsuoEdgeInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: TetsuoEdgeClassMetaBoxes
...
/*
   Class: TetsuoEdgeClassMetaBox
   A class that initializes Edge Meta Box
*/
class TetsuoEdgeClassMetaBox implements iTetsuoEdgeInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $tetsuo_edge_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: TetsuoEdgeClassTaxonomyOptions
   A class that initializes TetsuoEdgeClass Taxonomy Options
*/
class TetsuoEdgeClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: TetsuoEdgeClassTaxonomyOption
   A class that initializes TetsuoEdgeClass Taxonomy Option
*/
class TetsuoEdgeClassTaxonomyOption implements iTetsuoEdgeInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new TetsuoEdgeClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: TetsuoEdgeClassUserOptions
   A class that initializes TetsuoEdgeClass User Options
*/
class TetsuoEdgeClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: TetsuoEdgeClassUserOption
   A class that initializes TetsuoEdgeClass User Option
*/
class TetsuoEdgeClassUserOption implements iTetsuoEdgeInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new TetsuoEdgeClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: TetsuoEdgeClassDashboardOptions
   A class that initializes TetsuoEdgeClass Dashboard Options
*/
class TetsuoEdgeClassDashboardOptions {
	private static $instance;
...
/*
   Class: TetsuoEdgeClassDashboardOption
   A class that initializes TetsuoEdgeClass Dashboard Option
*/
class TetsuoEdgeClassDashboardOption implements iTetsuoEdgeInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new TetsuoEdgeClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'tetsuo_edge_init_framework_variable' ) ) {
	function tetsuo_edge_init_framework_variable() {
		global $tetsuo_edge_global_Framework;
		
		$tetsuo_edge_global_Framework = TetsuoEdgeClassFramework::get_instance();
		$edgtSkinManager                 = new TetsuoEdgeClassSkinManager();
		$tetsuo_edge_global_Framework->setSkin( $edgtSkinManager->getSkin() );
	}
	
	add_action( 'tetsuo_edge_action_before_options_map', 'tetsuo_edge_init_framework_variable' );
}
?>